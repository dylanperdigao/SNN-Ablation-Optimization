...
def metrics_performance_5fpr(y_true, y_prob):
    ...
    roc_auc_5fpr = roc_auc_score(y_true, y_prob, max_fpr=0.05)
    pr_precisions, pr_recalls, pr_thresholds = precision_recall_curve(y_true, y_prob)
    pr_auc = auc(pr_recalls, pr_precisions)
    metrics = {
        "roc_fprs": str(fprs),
        "roc_tprs": str(tprs),
        "roc_thresholds": str(thresholds),
        "roc_auc": roc_auc,
        "roc_auc_5fpr": roc_auc_5fpr,
        "pr_precisions": str(pr_precisions),
        "pr_recalls": str(pr_recalls),
        "pr_thresholds": str(pr_thresholds),
        "pr_auc": pr_auc,
